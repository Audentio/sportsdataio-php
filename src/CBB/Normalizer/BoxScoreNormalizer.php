...
class BoxScoreNormalizer implements DenormalizerInterface, NormalizerInterface, DenormalizerAwareInterface, NormalizerAwareInterface
{
    ...
    {
        return $type === 'Sportsdata\\API\\CBB\\Model\\BoxScore';
    }
    public function supportsNormalization($data, $format = null)
    {
        return is_object($data) && get_class($data) === 'Sportsdata\\API\\CBB\\Model\\BoxScore';
    }
    public function denormalize($data, $class, $format = null, array $context = array())
    {
        ...
        $object = new \Sportsdata\API\CBB\Model\BoxScore();
        if (null === $data || false === \is_array($data)) {
            return $object;
        }
        if (\array_key_exists('Game', $data)) {
            $object->setGame($this->denormalizer->denormalize($data['Game'], 'Sportsdata\\API\\CBB\\Model\\Game', 'json', $context));
        }
        ...
        if (\array_key_exists('PlayerGames', $data)) {
            ...
            foreach ($data['PlayerGames'] as $value_1) {
                $values_1[] = $this->denormalizer->denormalize($value_1, 'Sportsdata\\API\\CBB\\Model\\PlayerGame', 'json', $context);
            }
        ...
        if (\array_key_exists('TeamGames', $data)) {
            ...
            foreach ($data['TeamGames'] as $value_2) {
                $values_2[] = $this->denormalizer->denormalize($value_2, 'Sportsdata\\API\\CBB\\Model\\TeamGame', 'json', $context);
            }
