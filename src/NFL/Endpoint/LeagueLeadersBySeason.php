...
class LeagueLeadersBySeason extends \Sportsdata\API\NFL\Runtime\Client\BaseEndpoint implements \Sportsdata\API\NFL\Runtime\Client\Endpoint
{
    ...
    */
    public function __construct(string $season, string $column, string $position = 'ALL', string $format = 'JSON')
    {
        $this->season = $season;
        $this->column = $column;
    ...
    {
        return str_replace(array('{season}', '{column}', '{position}', '{format}'), array($this->season, $this->column, $this->position, $this->format), '/stats/{format}/SeasonLeagueLeaders/{season}/{position}/{column}');
    }
